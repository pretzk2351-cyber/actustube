...
> Production節は2026-07-27 JSTの確認に基づく履歴スナップショットです。2026-08-14のharness redesign自体ではGitHub Actions、Vercel、Neon、Google Cloudへ接続しておらず、Production / provider状態は再確認していません。その後、GitHub Actions run `32639709042`とphase observabilityを追加したrun `32684794700`はいずれもPostgreSQL 18.6 service-container gateで`RUN / FAILED`でした。後者はbroadなMigration postcondition markerまでをpublic-safeに特定しました。subphase化後のrun `32695896204`も`RUN / FAILED`で、固定markerはexplicit runtime executionを示しました。Migration本体とactual harness query pathの静的照合により、`SECURITY INVOKER`関数を実行するtemporary fixture roleにfunction bodyのobject privilegeが不足することをroot causeとして確定しました。raw PostgreSQL errorは根拠に使用していません。今回のfixture限定least-privilege body ACL修正commitに対するPostgreSQL 18.6 external gateはcommit時点で`NOT RUN`です。
