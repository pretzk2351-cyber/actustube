...
- local verification：修正前に実PostgreSQLへ通していたpreflight固有SQLはextension分類fragment中心で、identity、migration catalog、user-defined object catalog、before / afterの主要経路はmock fixtureでした。今回の修正ではrepositoryと同じ`embedded-postgres 18.4`をloopbackだけで使用し、preflight本体が実際に呼ぶproduction query関数を、pristine / exact empty migration table、database / role / server identity、extension inventory、migration column / exact catalog、user-defined object catalog、fresh before / after transactionへ通します。独立した第三sessionのcommitをfresh after snapshotが検出してexit 0を拒否するMVCC testも含めます。fixture PostgreSQLの子processにはDB、PostgreSQL、proxy、token、password、secret、credential、API key、auth系Environment Variableを継承させません。guardを明示したNode childではTCP / TLSの非loopback、DNS、UDP、非loopback listener、Windows remote named pipeを拒否します。guard未導入process、successful external connection、native child external connection、実package entrypoint、実Neon catalog／transaction pooler／staging owner／ACL、provider driverのnative cancellationはNOT VERIFIEDまたはNOT TESTEDであり、local PASSを実staging DB検証済みとは扱いません
