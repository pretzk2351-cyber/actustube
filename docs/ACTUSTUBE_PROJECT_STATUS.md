...
> Production節は2026-07-27 JSTの確認に基づく履歴スナップショットです。今回の2026-08-14 harness redesignではGitHub Actions、Vercel、Neon、Google Cloudへ接続しておらず、Production / provider状態は再確認していません。現在確認したGit状態、local unit検証と未実行のexternal fixture gateは本書末尾の候補節に記載します。
...
- Migration：履歴上の明示承認済みlocal使い捨てDBだけへ適用。新しいGitHub Actions external fixture gateは未実行であり、staging / Production DBへの適用なし
...
staging関連の次工程は、connection-only harness redesignのlocal検証と独立監査を完了し、明示承認後にfeature branchをpushしてGitHub Actions disposable PostgreSQL gateを初めて実行することです。workflowがPASSしても実staging preflightの代替にはせず、新しい完全HEADを基準に認証済みstaging構築の再開指示を別途受けます。今回の実装工程ではGitHub Actions、Docker、外部・staging・Production DB、実staging Migration / postflight、deployment、OAuth検証を実行しません。Production監視、期限付き例外の週次確認、plannedで残存するスモーク専用改善項目の判断も独立工程とし、データ変更、環境変数変更、Redeploy、rollbackには別の明示承認が必要です。
...
- direct／pooledはbeforeのread-only repeatable-read transactionを双方とも終了してから、after用のread-only repeatable-read transactionを双方で新しく開始します。GitHub Actionsのdigest固定PostgreSQL 18.6 fixtureでは第三sessionによるcommitをafterで検出し、同一snapshotを再読してdriftを見逃さないことをgateします。このexternal gateは現在NOT RUNです。
...
local unit testは外部fixture接続成功、PostgreSQL 18.6 native挙動、Migration、postflightを証明しません。それらはGitHub Actions workflowの実行後にだけ判定し、workflow未実行の現時点ではNOT RUNです。Next.js、npm、Vitest、Corepackその他のprocessがsensitive environment fileを読み取らなかったことも証明しません。
