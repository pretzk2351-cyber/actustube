...
> Production節は2026-07-27 JSTの確認に基づく履歴スナップショットです。run `33347045917`はhistorical automatic runとして`RUN / FAILED`です。current pre-fix HEAD `26e8a306fc5643a5d4a709bd6ade1ca79537a862`に対するautomatic push run `33349418965`もPostgreSQL 18.6 external fixtureで`RUN / FAILED`、failed stepはexternal disposable PostgreSQL verifierでした。public-safe aggregateはfull comparison `18 / 18 / 18 / 18 / 0`、grantor identityだけまたはgrantor dependencyだけを除外してもmissing / extraは各18、grantor domain全体を除外すると各0、fixed target relationはexpected 0 / actual 18、original observed relationはexpected 18 / actual 0、cleanupは`SUCCEEDED`です。read-only root-cause decisionはexpected oracle defectを`PROVEN`とし、execution authority defect、inventory attribution defect、今回のsuperuser branchにおけるnondeterministic selectionを`RULED_OUT`としました。今回のcandidateはexpected stored grantorを固定Migration ownership contract由来のper-object ownerへ、ownerのgrantor-side ACL dependency期待値を0へ修正します。GRANT / REVOKE / inventory SQL、query / operation count、deadline / cleanup、Migration、workflowは変更しません。修正後のPostgreSQL 18.6 PASSはcommit時点で`NOT VERIFIED`、actual staging / Production / Neon / poolerは`NOT VERIFIED`、Production Migration / deploymentは`NOT RUN`です。
