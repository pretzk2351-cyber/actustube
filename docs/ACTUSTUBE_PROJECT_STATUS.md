...
> Production節は2026-07-27 JSTの確認に基づく履歴スナップショットです。2026-08-14のharness redesign自体ではGitHub Actions、Vercel、Neon、Google Cloudへ接続しておらず、Production / provider状態は再確認していません。その後、GitHub Actions run `32639709042`のPostgreSQL 18.6 service-container gateは`RUN / FAILED`、phase observabilityを追加したrun `32684794700`も`RUN / FAILED`でした。後者では固定known marker `EXTERNAL_FIXTURE_VERIFICATION_FAILED_PHASE_MIGRATION_POSTCONDITIONS`がexact 1件だけ観測され、public-safe observabilityはqualifiedですが、同phaseが3箇所で再利用されていたためexact call siteとunderlying root causeは`NOT IDENTIFIED`です。今回分割するsubphase-marker commitのexternal gateはcommit時点で`NOT RUN`です。
...
- Migration：履歴上の明示承認済みlocal使い捨てDBだけへ適用。GitHub Actions run `32684794700`ではdisposable service-containerのMigration postcondition phaseへ到達しましたが、3箇所のどのsubphaseかと後続cleanup / postflightの状態は特定できません。実staging / Production DBへのMigration適用は0回で、新subphase-marker commitのqualification runはNOT RUNです
...
staging関連のfeature branch initial push、run `32639709042`のgeneric failure、phase observabilityを追加したrun `32684794700`のknown `MIGRATION_POSTCONDITIONS` failureまで完了しました。後者はpublic-safe marker exact 1件でphaseをlocalizedしましたが、markerが3箇所へ再利用されていたためexact call siteとunderlying root causeは`NOT IDENTIFIED`です。次工程はこのphaseだけをactual await境界ごとの固定subphaseへ分割するexact-scope変更、local validation、独立read-only review、normal feature-branch push exact 1回、新HEADのautomatic workflow監視です。新CIでsubphaseが判明してもunderlying defectを同じbatch内で修正せず、compare、Draft PR、ready-for-review、merge、staging構築、Production、provider、deploymentへ進みません。新subphase-marker commitのqualification runはcommit時点でNOT RUNです。
...
- direct／pooledはbeforeのread-only repeatable-read transactionを双方とも終了してから、after用のread-only repeatable-read transactionを双方で新しく開始します。GitHub Actionsのdigest固定PostgreSQL 18.6 fixtureでは第三sessionによるcommitをafterで検出し、同一snapshotを再読してdriftを見逃さないことをgateします。run `32684794700`は`RUN / FAILED`で`MIGRATION_POSTCONDITIONS`を観測しましたがexact call siteは`NOT IDENTIFIED`です。subphase instrumentationを含む新HEADのqualification runは現在NOT RUNです。
...
local unit testは外部fixture接続成功、PostgreSQL 18.6 native挙動、Migration、postflightを証明しません。それらのPASSはGitHub Actions workflowが成功した場合だけ判定できます。run `32684794700`は`RUN / FAILED`で、known markerは`MIGRATION_POSTCONDITIONS`を示しましたがexact subphaseとunderlying root causeは特定できませんでした。今回のsubphase observability-only変更はunderlying PostgreSQL defectを修正済みとは扱わず、新HEADのqualification runはNOT RUNです。Next.js、npm、Vitest、Corepackその他のprocessがsensitive environment fileを読み取らなかったことも証明しません。
