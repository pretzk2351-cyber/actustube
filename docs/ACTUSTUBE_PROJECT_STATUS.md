...
> Production節は2026-07-27 JSTの確認に基づく履歴スナップショットです。2026-08-14のharness redesign自体ではGitHub Actions、Vercel、Neon、Google Cloudへ接続しておらず、Production / provider状態は再確認していません。その後、GitHub Actions run `32639709042`でPostgreSQL 18.6 service-container gateを実行し、結果は`RUN / FAILED`でした。observed public markerは固定generic marker `EXTERNAL_FIXTURE_VERIFICATION_FAILED`で、underlying failure phaseは`NOT IDENTIFIED`です。現在確認したGit状態、local unit検証、このfailed run、新しいpublic-safe phase-marker qualification runの`NOT RUN`状態を本書末尾の候補節に記載します。
...
- Public-safe phase observability candidate：今回の変更はactual production external-harness pathへ固定ASCII phase markerを追加するobservability-only変更です。初期fixture Clientのconnect / close、Migration Clientのconnect / close、canonical cleanup Clientのconnect / closeは、それぞれ実在するlifecycle await boundaryとして固定phaseへ分類し、Client、query、retry、timerは追加しません。underlying PostgreSQL behavior、SQL、query count / order、Migration、ownership、ACL、cleanup、deadline、Client lifecycleは修正対象にしません。新しいphase-marker qualification runはcommit時点でNOT RUNです。raw error、error message、stack、cause、SQL、URL、credential、host、port、database、role、OID、ACL / catalog rowをpublic outputへ追加しません。fake Clientはactual PostgreSQL semanticsの証明ではなく、actual Neon / staging / Production / poolerはNOT VERIFIED、actual staging preflight / Migration / postflightはNOT RUN、repository-owned PostgreSQL lifecycle authorityは0です。
...
- Migration：履歴上の明示承認済みlocal使い捨てDBだけへ適用。GitHub Actions run `32639709042`ではexternal verifier stepが開始されましたが、generic markerのためdisposable service-container内のMigration phaseへの到達・完了状態はUNKNOWNです。実staging / Production DBへのMigration適用は0回で、新しいphase-marker qualification runはNOT RUNです
...
staging関連のfeature branch initial pushと最初のGitHub Actions PostgreSQL 18.6 service-container runは完了し、run `32639709042`は`RUN / FAILED`でした。observed markerはgeneric `EXTERNAL_FIXTURE_VERIFICATION_FAILED`でunderlying failure phaseを特定できなかったため、次工程はpublic-safe phase observabilityだけを追加するexact-scope変更、local validation、独立read-only review、normal feature-branch push exact 1回、新HEADのautomatic workflow監視です。新CIでphaseが判明してもunderlying defectを同じbatch内で修正せず、compare、Draft PR、ready-for-review、merge、staging構築、Production、provider、deploymentへ進みません。新しいphase-marker qualification runはこのcommit時点でNOT RUNです。
...
- direct／pooledはbeforeのread-only repeatable-read transactionを双方とも終了してから、after用のread-only repeatable-read transactionを双方で新しく開始します。GitHub Actionsのdigest固定PostgreSQL 18.6 fixtureでは第三sessionによるcommitをafterで検出し、同一snapshotを再読してdriftを見逃さないことをgateします。このexternal gateのrun `32639709042`は`RUN / FAILED`で、generic markerのためfailure phaseは`NOT IDENTIFIED`です。public-safe phase-marker instrumentationを含む新HEADのqualification runは現在NOT RUNです。
...
local unit testは外部fixture接続成功、PostgreSQL 18.6 native挙動、Migration、postflightを証明しません。それらのPASSはGitHub Actions workflowが成功した場合だけ判定できます。run `32639709042`は`RUN / FAILED`であり、generic marker `EXTERNAL_FIXTURE_VERIFICATION_FAILED`からunderlying failure phaseは特定できませんでした。今回のobservability-only変更はunderlying PostgreSQL defectを修正済みとは扱わず、新しいphase-marker qualification runはNOT RUNです。Next.js、npm、Vitest、Corepackその他のprocessがsensitive environment fileを読み取らなかったことも証明しません。
