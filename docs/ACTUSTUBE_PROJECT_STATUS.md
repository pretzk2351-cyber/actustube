...
- local verification：repositoryと同じ`embedded-postgres 18.4`をloopbackだけで使用し、preflight本体のproduction query関数をdatabase / role / server identity、extension inventory、migration column / exact catalog、user-defined object catalog、fresh before / after transactionへ通します。fixture lifecycleはproduction codeから到達しない専用Node harnessへ隔離し、親Vitestがoccurrence ID、harness PID、temporary root / data directory、port、stateを所有します。harness、PostgreSQL、Windows cleanup child、cleanup probeは同じpositive OS environmentだけを継承し、親`process.env`を全面変更しません。initialise / spawn / ready / execution / stop / forced cleanupに固定期限を持ち、ready前hang、partial-start throw、stop pending、child crashをactual subprocessで発生させます。異常時はoccurrence、exact PID、親子関係、root、portを照合し、process / listener消滅後だけdata directoryを削除し、unrelated sentinelは終了しません。独立した第三sessionのcommitをfresh after snapshotが検出してexit 0を拒否するMVCC testも維持します。guard未導入process、successful external connection、native child external connection、実package entrypoint、実Neon catalog／transaction pooler／staging owner／ACL、provider driverのnative cancellationはNOT VERIFIEDまたはNOT TESTEDであり、local PASSを実staging DB検証済みとは扱いません
...
- local PostgreSQL test harnessはcaller指定rootを廃止し、親の固定prefix `mkdtemp` root、作成時`lstat` / `realpath` identity、occurrence ID、one-time capability、Node IPCの初回exact initへ束縛します。direct `--child` / `--root` / unknown / duplicate invocation、wrong / duplicate IPC init、root交換、junction / symlinkをpre-mutationまたはpre-deleteで拒否し、process / listener消滅後にowned rootだけをentry単位で安全に削除します。
- Windows utilityはcallerの`SYSTEMROOT` / `WINDIR` / `PATH`をauthorityにせず、実行中Node executableのcanonical volume上の固定Windows system directoryとregular executable identityを検証してabsolute path、argument array、`shell: false`で起動します。childの`PATH`は検証済みsystem directoryだけ、`TEMP` / `TMP`はowned root配下へ固定し、秘密・DB・proxy・`NODE_OPTIONS`系keyを除外します。authority不明時はfallbackせず停止します。
- forced cleanupは全process終了、listener待ち、probe、utility、安全なdirectory削除で共有する単一monotonic 10秒deadlineです。harnessはChildProcess状態、PID、creation generation、canonical executable、parent PID、occurrence / IPC identityをkill直前に再照合し、不一致ならkill 0でtestを失敗させます。`ready_hang`、`partial_start_throw`、`stop_hang`、`child_crash`はmode別phase / terminal reasonとnegative controlをexact検証し、cleanup timerはclose / timeout双方で解除します。
