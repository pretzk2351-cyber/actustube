...
- local verification：repositoryと同じ`embedded-postgres 18.4`をloopbackだけで使用し、preflight本体のproduction query関数をdatabase / role / server identity、extension inventory、migration column / exact catalog、user-defined object catalog、fresh before / after transactionへ通します。fixture lifecycleはproduction codeから到達しない専用Node harnessへ隔離し、親Vitestがoccurrence ID、harness PID、temporary root / data directory、port、認証済みIPC phase、in-memory process ownership registryを所有します。filesystem上のlifecycle state fileは使用せず、PID／kill／delete authorityをfile contentから生成しません。harness、PostgreSQL、cleanup worker、cleanup probeはpositive OS environmentだけを継承し、親`process.env`を全面変更しません。initialise / spawn / ready / execution / stop / forced cleanupに固定期限を持ち、ready前hang、partial-start throw、stop pending、child crashをactual subprocessで発生させます。異常時は親が観測したexit／signal／deadlineと認証済みphase、spawn時・kill直前のprocess identityを照合し、process / listener消滅後だけowned rootをquarantineして削除し、unrelated sentinelは終了しません。独立した第三sessionのcommitをfresh after snapshotが検出してexit 0を拒否するMVCC testも維持します。guard未導入process、successful external connection、native child external connection、実package entrypoint、実Neon catalog／transaction pooler／staging owner／ACL、provider driverのnative cancellationはNOT VERIFIEDまたはNOT TESTEDであり、local PASSを実staging DB検証済みとは扱いません
...
staging関連の次工程は、Migration前read-only preflight実装の独立監査とGit確定を完了し、その新しい完全HEADを基準に認証済みstaging構築の再開指示を新たに承認することです。preflight exit code 0までは実staging Migrationへ進まず、今回の実装工程では外部・staging・Production DB接続、実staging Migration / postflight、deployment、OAuth検証を行いません。明示承認されたloopback上の使い捨てlocal fixtureだけでPostgreSQL 18.4 preflightとMigration 0000〜0006後のpostflight harnessを検証し、終了後にprocess、listener、data directoryを削除します。Production監視、期限付き例外の週次確認、plannedで残存するスモーク専用改善項目のcleanup判断も独立した工程として扱い、データ変更、環境変数変更、Redeploy、rollbackを必要とする場合は別の明示承認を受けます。
...
- direct／pooledはbeforeのread-only repeatable-read transactionを双方とも終了してから、after用のread-only repeatable-read transactionを双方で新しく開始します。実PostgreSQL 18.4の第三sessionによるcommitをafterで検出するため、同一snapshotを再読してdriftを見逃す構成ではありません。
...
- local PostgreSQL test harnessはcaller指定rootを廃止し、親の固定prefix `mkdtemp` root、作成時`lstat` / `realpath` identity、occurrence ID、harness用とcleanup用の別one-time capability、private Node IPCの初回exact initへ束縛します。direct `--child` / `--root` / unknown / duplicate invocation、wrong / duplicate IPC init、root交換をpre-mutationで拒否します。cleanupは同一temporary parent内のrandom quarantine名へrootをatomic renameし、entry identityを各operation直前に再検証します。junction / symlinkはtargetを辿らずlink entryだけを除去し、hardlink contentを書き換えず、identityが変化したentryは削除せずcleanup incompleteにします。
- Windows utilityはcallerの`SYSTEMROOT` / `WINDIR` / `PATH`をauthorityにせず、実行中Node executableのcanonical volume上でdrive root、Windows、System32、WindowsPowerShell、v1.0、executableのlexical全componentを`lstat` / `realpath`検証します。中間reparse、canonical不一致、type／parent不一致を拒否し、regular executableをabsolute path、argument array、`shell: false`で起動します。childの`PATH`は検証済みsystem directoryだけ、`TEMP` / `TMP`はowned root配下へ固定し、秘密・DB・proxy・`NODE_OPTIONS`系keyを除外します。authority不明時はfallbackせず停止します。
- forced cleanupはprivate IPCの専用cleanup workerへ隔離し、親が作成する単一monotonic 10秒deadline内でprocess query／kill、listener、probe、utility、quarantine、filesystem cleanupを管理します。cancel不能operationが停止した場合は、親が記録済みChildProcessとexact worker identityを照合してreserve内にworker treeを終了し、deadline後の追加probe／delete／killを開始せずcleanup incompleteを返します。descendant authorityは認証済みspawn IPCを親がOS側identityで独立確認したin-memory registryだけであり、state fileは存在しません。`ready_hang`、`partial_start_throw`、`stop_hang`、`child_crash`のterminal reasonは親がexit／signal／deadline／alive／closeとordered phaseから生成し、real timer testはclose／timeout／reject／abort後のtimerとlistener解除を確認します。
