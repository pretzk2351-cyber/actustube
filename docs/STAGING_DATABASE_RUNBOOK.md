...
body ACL inventoryはfixed recipient role scopeとは別の必須parameterでoriginal DB-observed grantorを受け取り、fixed objectsについてexpected recipientがgranteeであるACL、またはoriginal observed grantorから全non-owner recipientへのACLを列挙します。unknown third recipient、wrong grantor / grantee、grantor-side shared dependency、cleanup後のunknown ACL / dependency residueはexpected 18 rowsへ追加せずfail-closedに拒否します。test-local REVOKE oracleはGRANT / REVOKE actionを区別し、object kind、privilege set / order、recipient、grantor、statement order / presence、trailing SQL、separate parameter、non-array parameter、QueryConfig values、action swap、duplicateを独立mutationとして実差分確認します。fixed recipient以外とunrelated authorityを誤って削除しません。fake Clientはactual PostgreSQL semanticsを証明しません。combined two-commit PostgreSQL 18.6 gateは本follow-up commit時点で`NOT RUN`、actual staging / Production / Neon / poolerは`NOT VERIFIED`、Production Migration / deploymentは`NOT RUN`、repository-owned PostgreSQL lifecycle authorityは0です。
...
修正candidateは、final Migrationとowner postcondition、replayとowner postconditionの完了後だけfixed immutable manifestからtemporary body-object ACLを付与します。direct recipientはexplicit runtime fixture roleとmembership runtime groupだけで、membership leafはgroup inheritanceだけを使います。各fresh GRANT / REVOKE Clientはidentity-firstでoriginal DB-observed frozen session identityとの一致を検証し、各statementのgrantorもこのauthorityへ固定してconfiguration / caller roleへfallbackしません。denied role、PUBLIC、Production runtime role、Migration executor、legacy ownerへのgrantは0です。GRANT後はfixed manifestから生成した18-row expected contractと、それとは別に取得したobserved-grantor complete catalog inventoryを完全照合し、grantor、grantee、grant option、direct relation、`pg_shdepend.deptype = 'a'` coverageを確認してからだけusage executionへ進みます。usage / plan / reservation verificationはreserved cleanup boundary内で実行し、成功、assertion failure、query rejection、timeoutの各経路でfresh cleanup Clientによるactual bounded REVOKEを試行します。REVOKE後は同じcomplete inventoryのexact empty setを要求し、unknown recipientを含むgrantor / grantee / ACL dependency residueまたはcleanup未確認があれば`configureRuntimeAcl()`、canonical cleanup、postflightを開始しません。既存absolute deadlineを延長・resetせず、broad grant / cleanup、`DROP OWNED`、`REASSIGN OWNED`を追加しません。

このcandidateとfollow-upはharness、local fake oracle、本Runbook、Project Statusだけを変更し、Migration、application runtime、workflowは変更しません。test-local action-aware SQL / parameter oracleとfake actual ACL stateはproduction manifestから独立していますが、fake Client testはactual PostgreSQL semanticsを証明しません。combined two-commit PostgreSQL 18.6 external gateは本follow-up commit時点で`NOT RUN`、actual staging / Production / Neon / poolerは`NOT VERIFIED`、Production Migration / deploymentは`NOT RUN`、repository-owned PostgreSQL lifecycle authorityは0です。
...
- external fixture separate-role connection、PostgreSQL 18.6 actual結果、移植したusage Migration意味検証：run `32695896204`は`RUN / FAILED`、fixed markerはexplicit runtime executionで、静的root causeは`SECURITY INVOKER` body-object privilege不足。body ACL fixと本follow-upのcombined qualification runはcommit時点で`NOT RUN`
...
local fake Client / unit testのPASSはrole・owner・deadline・oracle境界の静的な実装確認であり、actual PostgreSQLのMigration実行結果ではありません。local unit PASSまたは将来のGitHub Actions disposable fixture PASSを、実provider poolerや実staging DBのPASSとして扱いません。run `32695896204`は`RUN / FAILED`であり、body ACL fixと本follow-upのcombined PostgreSQL 18.6 qualification runはcommit時点で`NOT RUN`です。次の外部qualification stepは、local validationとcombined two-commit independent reviewでblocking finding 0の場合だけ行うnormal feature-branch pushにより自動起動されるworkflowの監視です。manual dispatch、same-SHA rerun、Draft PR、staging / Production / provider操作へ進みません。
