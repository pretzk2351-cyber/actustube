...
connection configurationのroleは接続時のexpected identityにすぎません。最初のMigration ClientはGRANT / Migration開始前にDB-observed `session_user` / `current_user`をexact 1-row contractで検証し、actual `session_user`をimmutable internal grantor identityとして保持します。expected 11 ACL grantorはこのobserved identityから生成し、configuration roleやACL inventory resultから生成しません。cleanup Clientはtemporary-authority inventoryと最初のREVOKEより前にidentityを再観測し、両roleがfrozen observed identityと一致する場合だけ続行します。local testではcaller configuration、initial / cleanup DB-observed role、actual fake ACL grantorを独立したoption / state pathで供給します。このfake Client検証はactual PostgreSQLを証明せず、PostgreSQL 18.6 external gateはNOT RUN、実staging / Production / Neon / poolerはNOT VERIFIED、repository-owned PostgreSQL lifecycle authorityは0のままです。
