...
このthree-commit candidateはharness、local fake oracle、本Runbook、Project Statusだけを変更し、Migration、application runtime、workflowは変更しません。test-local action-aware SQL / parameter oracle、ACL selection authority、dependency-side catalog fixture、fake actual ACL stateはproduction manifest / matcherから独立していますが、fake Client testはactual PostgreSQL semanticsを証明しません。combined three-commit PostgreSQL 18.6 external gateはcandidate commit snapshot時点で`NOT RUN`、actual staging / Production / Neon / poolerは`NOT VERIFIED`、Production Migration / deploymentは`NOT RUN`、repository-owned PostgreSQL lifecycle authorityは0です。
...
- external fixture separate-role connection、PostgreSQL 18.6 actual結果、移植したusage Migration意味検証：run `32695896204`は`RUN / FAILED`、fixed markerはexplicit runtime executionで、静的root causeは`SECURITY INVOKER` body-object privilege不足。combined three-commit qualification runはcandidate commit snapshot時点で`NOT RUN`
