...
`tests/staging-database-preflight.test.ts`のlocal PostgreSQL検証は、production codeから到達しない専用Node harnessへ隔離します。親VitestがOS temporary directory直下へ固定prefix付きの専用rootを`mkdtemp`で新規作成し、作成時の`lstat` identityと`realpath`、unique occurrence ID、one-time capabilityを記録します。child CLIはroot、data directory、port、削除対象を受け取らず、親が開いたNode IPC channel上の初回exact initだけをroot claimのSHA-256と照合して受理します。IPCなしのdirect invocation、`--root`、unknown / duplicate argument、wrong / duplicate initはfile作成やprocess起動前に拒否します。rootは各書込み、PostgreSQL起動、cleanup、削除直前に再照合し、root交換、junction / symlink、repository / home / drive / OS temp root、root外entryをfail-closedで拒否します。

harness、PostgreSQL、通常stopが内部利用するWindows utility、cleanup environment probeには、親のsentinel用environmentとは別のpositive environmentだけを渡します。Windows utility authorityはcallerの`SYSTEMROOT` / `WINDIR` / `PATH`から選ばず、実行中Node executableのcanonical volume上にある固定`Windows\System32`系directoryを`lstat` / `realpath`で検証し、固定basenameのregular executableをabsolute path、argument array、`shell: false`で起動します。このauthorityを確定できなければPATHへfallbackせず停止します。childの`SYSTEMROOT` / `WINDIR`はそのcanonical root、`PATH`は検証済みSystem32 / WindowsPowerShell directoryだけ、`TEMP` / `TMP`はowned root配下へ固定し、DB / PostgreSQL / proxy / token / credential / secret / password / auth / `NODE_OPTIONS`を大小文字非依存で除外します。親test workerの`process.env`は変更しません。

repositoryと同じ`embedded-postgres 18.4`で、database / role / server identity、extension inventory、migration table / column inventory、exact migration catalog、user-defined object catalog / count、read-only transaction、fresh before / afterをproduction query実行関数へ通します。exact empty migration tableの安定状態はexit 0、独立した第三sessionのDDL commitはdriftとしてexit 0を拒否します。期限はinitialise 30秒、process spawn 5秒、ready 20秒、test execution 60秒、normal stop 10秒、fault phase 1.5秒です。forced cleanupは開始時の単一monotonic absolute deadlineから全stageへremaining budgetを渡し、harness / owned postmaster終了、process / listener消滅待ち、cleanup probe、utility、safe directory削除を合計最大10秒に制限します。harness kill前にはChildProcess状態、PID、creation generation、canonical executable、parent PID、occurrence / IPC関係をspawn時identityと再照合し、不一致や取得不能ならkillしません。owned postmasterも記録済みgenerationと一致する場合だけtree termination対象にします。processとlistenerの消滅後、symlink / junctionを辿らないentry単位の削除でowned rootだけを削除します。

fault testは`ready_hang`、`partial_start_throw`、`stop_hang`、`child_crash`ごとにordered phase sequence、`intended fault reached = true`、固定terminal reason、cleanup到達、process / listener / directory residue 0をexact比較します。目的phase前の別failureを与えるnegative control、PID generation / executable / parent不一致でkill 0、unrelated sentinel維持、global deadline後のutility / kill開始0、close / timeout双方のtimer解除を対象testで確認し、任意の非0終了を指定fault成功とは扱いません。実Neon DB、provider transaction pooler、実staging owner / ACL、native cancellation、external connection成功有無の包括的証明、Production環境は引き続きNOT TESTED / NOT VERIFIEDであり、local PASSをそれらの成功へ拡張しません。
