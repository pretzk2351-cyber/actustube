...
`tests/staging-database-preflight.test.ts`のlocal PostgreSQL検証は、production codeから到達しない専用Node harnessへ隔離します。親Vitestはunique occurrence ID、直下harness PID、専用temporary root / data directory、loopback port、lifecycle stateを所有し、harness内だけで`initialise`、`start`、database作成・削除、production query integration、第三session drift、`stop`、dependency exit-hook cleanupを実行します。harness、PostgreSQL、通常stopが起動するWindows `taskkill`、実spawnしたcleanup environment probeは、必要なWindows OS keyだけの同じpositive environmentを継承し、DB / PostgreSQL / proxy / token / credential / secret / `NODE_OPTIONS`を継承しません。親test workerの`process.env`は全面変更しません。

repositoryと同じ`embedded-postgres 18.4`で、database / role / server identity、extension inventory、migration table / column inventory、exact migration catalog、user-defined object catalog / count、read-only transaction、fresh before / afterをproduction query実行関数へ通します。exact empty migration tableの安定状態はexit 0、独立した第三sessionのDDL commitはdriftとしてexit 0を拒否します。期限はinitialise 30秒、process spawn 5秒、ready 20秒、test execution 60秒、normal stop 10秒、forced cleanup 10秒、fault phase 1.5秒です。ready前hang、partial-start throw、stop pending、child crashでは、状態ファイルのoccurrence、PID、親子関係、root、data directory、portを照合し、通常stopで完了しないexact owned PID treeだけを終了します。processとlistenerの消滅確認後にdata directoryを削除し、unrelated sentinel processは維持します。cleanup不完全はtest failureです。実Neon DB、provider transaction pooler、実staging owner / ACL、native cancellationはNOT TESTEDであり、local PASSをそれらの成功へ拡張しません。
