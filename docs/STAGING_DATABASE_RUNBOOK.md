...
修正candidateは、final Migrationとowner postcondition、replayとowner postconditionの完了後だけfixed immutable manifestからtemporary body-object ACLを付与します。direct recipientはexplicit runtime fixture roleとmembership runtime groupだけで、membership leafはgroup inheritanceだけを使います。各GRANT / REVOKEはoriginal DB-observed frozen session identityを`GRANTED BY`へ固定し、configuration / caller roleへfallbackしません。denied role、PUBLIC、Production runtime role、Migration executor、legacy ownerへのgrantは0です。GRANT後はfixed manifestから生成した18-row expected contractと、それとは別に取得したactual catalog inventoryを完全照合し、grantor、grantee、grant option、direct relation、`pg_shdepend.deptype = 'a'` coverageを確認してからだけusage executionへ進みます。usage / plan / reservation verificationはbounded cleanup boundary内で実行し、成功、assertion failure、query rejection、timeoutの各経路で同じfixed manifestのREVOKEを試行します。REVOKE後は同じcomplete inventoryのexact empty setを要求し、grantor / grantee / ACL dependency residueまたはcleanup未確認があれば`configureRuntimeAcl()`、canonical cleanup、postflightを開始しません。既存absolute deadlineを延長せず、broad grant / cleanup、`DROP OWNED`、`REASSIGN OWNED`を追加しません。

このcandidateはharnessとlocal fake oracleおよび本Runbook / Project Statusだけを変更し、Migration、application runtime、workflowは変更しません。test-local SQL / parameter oracleとfake actual ACL stateはproduction manifestから独立していますが、fake Client testはactual PostgreSQL semanticsを証明しません。新commitのPostgreSQL 18.6 external gateはcommit時点で`NOT RUN`、actual staging / Production / Neon / poolerは`NOT VERIFIED`、Production Migration / deploymentは`NOT RUN`、repository-owned PostgreSQL lifecycle authorityは0です。
...
- external fixture separate-role connection、PostgreSQL 18.6 actual結果、移植したusage Migration意味検証：run `32684794700`は`RUN / FAILED`。`MIGRATION_POSTCONDITIONS`まではlocalizedしましたがexact subphase、cleanup / postflightの完了状態、root causeはNOT IDENTIFIEDで、PASSはNOT VERIFIED。新subphase-marker commitのqualification runはNOT RUN
...
local fake Client / unit testのPASSはrole・owner・deadline境界の静的な実装確認であり、actual PostgreSQLのMigration実行結果ではありません。local unit PASSまたは将来のGitHub Actions disposable fixture PASSを、実provider poolerや実staging DBのPASSとして扱いません。run `32684794700`は`RUN / FAILED`でありexternal fixture検証をPASSと記録しません。broad markerだけではexact subphaseまたはroot causeを確定せず、新subphase-marker commitのqualification runは実行前のためNOT RUNです。
