...
read-only root-cause decisionはexpected oracle defectを`PROVEN`とし、execution authority defect、inventory attribution defect、今回のsuperuser branchにおけるnondeterministic selectionを`RULED_OUT`としました。PostgreSQL 18ではsuperuserによるobject privilege GRANTは対象object ownerからのgrantとしてstored ACLへ記録され、object ownerはACL dependencyではなくownership dependencyを持ちます。修正candidateはfixed 5-object manifestの各objectを既存Migration ownership contractのper-object ownerへ明示的に束縛し、expected 18-rowのgrantorをobject owner、grantor-side ACL dependencyを0とします。original DB-observed authorityはfresh GRANT / REVOKE Client identity gateと既存compatibility inputだけに維持します。GRANT / REVOKE / inventory SQL、QueryConfig / parameter、query / Client operation count、total deadline 300,000ms、cleanup reserve 105,000ms、cleanup、Migration、workflowは変更しません。test-local multi-owner oracleはglobal scalarとcross-object substitutionを拒否しますが、fake Clientはactual PostgreSQL semanticsの証明ではありません。修正後のPostgreSQL 18.6 external gateはcommit時点で`NOT RUN`、actual staging / Production / Neon / poolerは`NOT VERIFIED`、Production Migration / deploymentは`NOT RUN`です。
...
local fake Client / unit testのPASSはrole・owner・deadline・oracle境界の静的な実装確認であり、actual PostgreSQLのMigration実行結果ではありません。local unit PASSまたは将来のGitHub Actions disposable fixture PASSを、実provider poolerや実staging DBのPASSとして扱いません。run `32695896204`、run `33291908052`、run `33347045917`、run `33349418965`はいずれもhistorical `RUN / FAILED`です。次の外部qualification stepは、expected owner oracle candidateのlocal validationと独立review 2系統でP0 / P1 / P2とblocking NOT VERIFIEDが0の場合だけ行うnormal feature-branch pushにより自動起動されるworkflowの監視です。full CI successの場合だけread-only remote compareへ進みます。CI failure時は追加fix、追加commit、rerun、same-SHA repushへ進みません。manual dispatch、Draft PR、staging / Production / provider操作へ進みません。
