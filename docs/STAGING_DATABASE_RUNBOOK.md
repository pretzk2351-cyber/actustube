...
local fake Client / unit testのPASSはrole・owner・deadline・oracle境界の静的な実装確認であり、actual PostgreSQLのMigration実行結果ではありません。local unit PASSまたは将来のGitHub Actions disposable fixture PASSを、実provider poolerや実staging DBのPASSとして扱いません。run `32695896204`、run `33291908052`、run `33347045917`、run `33349418965`、run `33584551679`はいずれもhistorical `RUN / FAILED`です。次の外部qualification stepは、reservation-concurrency setup observability candidateのlocal validationと独立review 2系統でP0 / P1 / P2とblocking NOT VERIFIEDが0の場合だけ行うnormal feature-branch pushにより自動起動されるworkflowの監視です。同じphaseが失敗した場合はfixed branch categoryだけを取得して追加fixせず停止し、full CI successの場合だけread-only remote compareへ進みます。CI failure時は追加fix、追加commit、rerun、same-SHA repushへ進みません。manual dispatch、Draft PR、staging / Production / provider操作へ進みません。
